...
export class MailView implements CurrentView {
	...
	async _getShortcuts(): Promise<Array<Shortcut>> {
		...
		if (isCustomizationEnabledForCustomer(customer, FeatureType.Newsletter)) {
			const mailboxDetails = await this._getMailboxDetails()
			shortcuts.push({
				key: Keys.P,
				exec: () => {
					import("../press/PressReleaseEditor").then(({openPressReleaseEditor}) => openPressReleaseEditor(mailboxDetails))
					return true
				},

...
export class BuildServer {
	...
	async _connectionListener(socket) {
		socket.on("data", async (data) => this._onData(data, (...args) => this._logTee(args)))
		      .on("error", async (data) => this._onError(data, (...args) => this._logTee(args)))
		      .on("close", async (data) => this._onClose(data, (...args) => this._logTee(args)))
	}
	...
	_startDevServer() {
		if (!this.webRoot) {
			this.log("Not root path defined, not starting devServer")
			return
		}
		const app = express()
		const sockets = []
